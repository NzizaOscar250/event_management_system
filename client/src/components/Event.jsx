...
const Event = ({ backgroundImage, title, creatorUsername, startDate, startTime, endTime, takenSpot, availableSpot,location,id }) => {
    ...
    <Link to={`events/${id}`}>
      ...
      <div className="relative z-10 flex gap-2">
        ...
        <div className="px-3">
          <h2 className="text-md font-bold pt-2">{truncatedTitle}</h2>
          <p className="text-gray-700 text-xs mb-2">@{creatorUsername}</p>
          <div className="text-xs">
            <p className='flex gap-1 font-bold'><span  className="text-blue-500">{availableSpot - takenSpot}</span> Available Spots </p>
          </div>
        </div>
        ...
        <div className='flex gap-2 items-center justify-center'>
                    ...
                    {moment(startDate).format('DD MMMM YYYY')}, {startTime} - {endTime}</p>
        </div>
